...
int read_cache_L1(hwaddr_t addr) {
    ...
    uint32_t tag = (addr >> (CACHE_b + CACHE_L1_s)); //标签

    int num;
    int set_begin = set * CACHE_L1_E;
    int set_end = (set + 1) * CACHE_L1_E;

    for (num = set_begin; num < set_end; num++)
        if (cache_L1[num].valid_bit && cache_L1[num].tag == tag) // Hit!
            return num;
    ...
    for (num = set_begin; num < set_end; num++)
        if (!cache_L1[num].valid_bit)
            break;

    if (num == set_end)
        num = set_begin + rand() % CACHE_L1_E;

    memcpy(cache_L1[num].data, cache_L2[block_i_L2].data, CACHE_B);
    
    cache_L1[num].valid_bit = true;
    cache_L1[num].tag = tag;
    return num;
}
...
int read_cache_L2(hwaddr_t addr) {
    ...
    for (num = set_begin; num < set_end; num++)
        if (cache_L2[num].valid_bit && cache_L2[num].tag == tag) 
            return num; 

    srand(time(0));
    for (num = set_begin; num < set_end; num++)
        if (!cache_L2[num].valid_bit)
            break;

    if (num == set_end)
        num = set_begin + rand() % CACHE_L2_E;

    int i;
    if (cache_L2[num].valid_bit && cache_L2[num].dirty_val) {
        uint8_t tmp[BURST_LEN << 1];
        memset(tmp, 1, sizeof(tmp));
        uint32_t block_start_x = (cache_L2[num].tag << (CACHE_L2_s + CACHE_b)) | (set << CACHE_b);

        for (i = 0; i < CACHE_B / BURST_LEN; i++) {
            ddr3_write_me(block_start_x + BURST_LEN * i, cache_L2[num].data + BURST_LEN * i, tmp);
        }
    }

    for (i = 0; i < CACHE_B / BURST_LEN; i++) {
        ddr3_read_me(block_start + BURST_LEN * i, cache_L2[num].data + BURST_LEN * i);
    }

    cache_L2[num].valid_bit = true;
    cache_L2[num].dirty_val = false;
    cache_L2[num].tag = tag;
...
void write_cache_L1(hwaddr_t addr, size_t len, uint32_t data) {
    ...
    for (num = set_begin; num < set_end; num++) {
        if (cache_L1[num].valid_bit && cache_L1[num].tag == tag) {
            if (block_bias + len > CACHE_B) {
                dram_write(addr, CACHE_B - block_bias, data);
                memcpy(cache_L1[num].data + block_bias, &data, CACHE_B - block_bias);
                ...
                memcpy(cache_L1[num].data + block_bias, &data, len);
...
void write_cache_L2(hwaddr_t addr, size_t len, uint32_t data) {
    ...
    for (num = set_begin; num < set_end; num++) {
        if (cache_L2[num].valid_bit && cache_L2[num].tag == tag) {
            ...
            cache_L2[num].dirty_val = true;
            if (block_bias + len > CACHE_B) {
                memcpy(cache_L2[num].data + block_bias, &data, CACHE_B - block_bias);
                write_cache_L2(addr + CACHE_B - block_bias, len - (CACHE_B - block_bias), data >> (CACHE_B - block_bias));
            } 
            else {
                memcpy(cache_L2[num].data + block_bias, &data, len);
            }
            return;
        }
    }

    num = read_cache_L2(addr);
    cache_L2[num].dirty_val = true;
    memcpy(cache_L2[num].data + block_bias, &data, len);
    return;
}